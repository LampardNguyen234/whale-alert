...
func (msg RelayerAddedMsg) String() string {
	msgFormatter := new(processorCommon.MsgFormatter).
		FormatTitle("Bridge Relayer Added").
...
func (p *BridgeProcessor) processRelayerAdded(_ context.Context, eventMsg EventMsg) error {
	msg := RelayerAddedMsg{
		Relayer: eventMsg.Event.(*contracts.BridgeRelayerAdded).Relayer.String(),
